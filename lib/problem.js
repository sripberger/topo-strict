...
export class Problem extends Validatable {
	...
	toString() {
		const lines = [];
		if (!_.isEmpty(this.ids)) lines.push(this._getIdsString());
		if (!_.isEmpty(this.groups)) lines.push(this._getGroupsString());
		return _.isEmpty(lines) ? 'Empty problem' : lines.join('\n\n');
	}
	...
	_getIdsString() {
		const lines = [ 'ids', '---' ];
		for (const id of _.keys(this.ids).sort()) {
			const { before = [], after = [] } = this.ids[id];
			lines.push(id);
	...
	_getGroupsString() {
		const lines = [ 'groups', '------' ];
		for (const key of _.keys(this.groups).sort()) {
