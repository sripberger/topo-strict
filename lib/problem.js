...
export class Problem extends Validatable {
	...
	toString() {
		const sections = [];
		const { _ids, _groups } = this;
		if (!_.isEmpty(_ids)) sections.push(getIdsSection(_ids));
		if (!_.isEmpty(_groups)) sections.push(getGroupsSection(_groups));
 ...
 * @private
 * @param {Object} ids - Constraint objects for ids, keyed by id.
 ...
 * @private
 * @param {Object} groups - Grouped ids, keyed by group key.
