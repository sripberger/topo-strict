...
export class Problem {
	...
	_getKeyCollisionErrors(values, groupKey) {
		const errors = this._getValueCollisionErrors(values);
	...
	_getValueCollisionErrors(values) {
	...
	_getExistingValueErrors(values) {
		return _.intersection(_.uniq(values), _.keys(this.items))
			.map((value) => {
	...
	_getExistingGroupErrors(values) {
		return _.intersection(_.uniq(values), _.keys(this.groups))
			.map((value) => {
				return new ArgumentError(
